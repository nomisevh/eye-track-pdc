...
def main(seed, fixed=True, use_cached_features=True, percentage=0.01, trade_off_coef=0.1):
    ...
    # Changed this to explore different alpha values
    ...
    #print(type(train_batch.x))
    #print(train_batch.x.shape)
    #train_val_batch = torch.utils.data.ConcatDataset([train_batch, val_batch])
    ...
    try:
        ...
        train_val_features = torch.load(features_filename('train_val'))
        # Load rocket mean and std
        with open(rocket_instances_path.joinpath(f'rocket_normalization_{seed}.pkl'), 'rb') as reader:
        ...
        torch.save(train_val_features, features_filename('train_val'))
    ...
    train_val_pred = ridge_clf.predict(train_val_features)
    ...
    train_score = balanced_accuracy_score(train_val_labels, train_val_pred)
    ...
    if fixed:
        ...
        optimal_index = (np.abs(percentage_vector - fixed_percentage)).argmin()
        optimal_percentage = percentage_vector[optimal_index]
        ...
        optimal_index, optimal_percentage = select_optimal_model(percentage_vector, score_list_test, score_list_test[0],acc_size_tradeoff_coef=trade_off_coef)
    ...
    # Print Number of selected features at the optimal index
    print(' ')
    print('Number of selected features: ' + str(np.sum(feature_selection_matrix[optimal_index])))
    
    # Print number of elements of the feature importance matrix greater than zero at the optimal index
    print(' ')
    print('Feature importance Matrix elements greater than zero at the optimal index: '+ str(np.sum(feature_importance_matrix[optimal_index] > 0)))
    ...
    with open(rocket_instances_path.joinpath(f'all_kernels_{seed}.pkl'), 'wb') as writer:
        pickle.dump(rocket.convs, writer)
    with open(rocket_instances_path.joinpath(f'kept_kernels_{seed}.pkl'), 'wb') as writer:
        pickle.dump(kept_kernels, writer)
    
    # Save rocket mean and std
    rocket_params = {'mean': rocket.mean, 'std': rocket.std}
    with open(rocket_instances_path.joinpath(f'rocket_normalization_{seed}.pkl'), 'wb') as writer:
        pickle.dump(rocket_params, writer)
    ...
    # Save the pruned rocket model and the retrained classifier
    ...
    with open(rocket_instances_path.joinpath(f'pruned_rocket_clf_{seed}.pkl'), 'wb') as writer:
        ...
        # pickle.dump(retrained_clf[0], writer)
    ...
    # Save the pruned rocket mean and std
    pruned_rocket_params = {'mean': rocket.mean, 'std': rocket.std}
    with open(rocket_instances_path.joinpath(f'pruned_rocket_normalization_{seed}.pkl'), 'wb') as writer:
        pickle.dump(pruned_rocket_params, writer)
...
if __name__ == '__main__':
    for seed in [1,2,42,1337,9000]:
        print(f"Running with seed {seed}")
        #main(seed, use_cached_features=True, fixed=True, percentage=0.05)
        main(seed, use_cached_features=True, fixed=False, trade_off_coef=0.1)
    #main(9000, use_cached_features=True, percentage=0.025) #9000 #2